...
import 'package:pbl_mid/goal_chart//goal_data.dart';

// 꺾은 선 그래프
// 지정한 month 기준 최근 6개월 간의 달성률을 나타내는 그래프
// 기본 : 현재 날짜에 해당하는 month
// 앞, 뒤로 슬라이딩해서 범위 변경(추후 드롭다운으로 변경 가능)
// 그래프가 그려지는 애니메이션
...
class _GoalLineChartState extends State<GoalLineChart> {
  ...
  Future<void> _showMonthPicker(BuildContext context) async {
    ...
    final pickedDate = await showDatePicker(
      ...
      firstDate: firstDate,
      lastDate: lastDate,
      // DatePicker 위젯은 Day를 선택하지만, 저희는 Month/Year만 사용합니다.
      // Day 선택 후 확인 버튼을 누르면 이 값이 반환됩니다.

      builder: (BuildContext context, Widget? child) {
        return Theme(
          data: ThemeData.light().copyWith(
            colorScheme: ColorScheme.light(
              primary: PRIMARY_COLOR, // 헤더 배경색
              onPrimary: Colors.white, // 헤더 텍스트/아이콘 색상
              onSurface: Colors.black87, // 날짜 텍스트 색상
              surface: Colors.white, // 배경색
            ),
            dialogBackgroundColor: Colors.white,
            textButtonTheme: TextButtonThemeData(
              style: TextButton.styleFrom(
                foregroundColor: PRIMARY_COLOR, // 버튼 텍스트 색상 (확인, 취소)
              ),
            ),
          ),
          child: child!,
    ...
    if (pickedDate != null) {
      // 선택된 날짜를 기준으로 endIndex를 업데이트합니다.
